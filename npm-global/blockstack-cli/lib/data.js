...
function makeAssociationToken(appPrivateKey, identityKey) {
    const appPublicKey = utils_1.getPublicKeyFromPrivateKey(`${utils_1.canonicalPrivateKey(appPrivateKey)}01`);
    const FOUR_MONTH_SECONDS = 60 * 60 * 24 * 31 * 4;
    const salt = crypto.randomBytes(16).toString('hex');
    const identityPublicKey = utils_1.getPublicKeyFromPrivateKey(identityKey);
...
function gaiaConnect(network, gaiaHubUrl, privateKey, ownerPrivateKey) {
    const addressMainnet = network.coerceMainnetAddress(utils_1.getPrivateKeyAddress(network, `${utils_1.canonicalPrivateKey(privateKey)}01`));
    const addressMainnetCanonical = network.coerceMainnetAddress(utils_1.getPrivateKeyAddress(network, utils_1.canonicalPrivateKey(privateKey)));
    let associationToken;
    if (ownerPrivateKey) {
        associationToken = makeAssociationToken(privateKey, ownerPrivateKey);
    }
    return blockstack.connectToGaiaHub(gaiaHubUrl, utils_1.canonicalPrivateKey(privateKey), associationToken)
        .then((hubConfig) => {
 ...
 */
function makeZoneFileFromGaiaUrl(network, name, gaiaHubUrl, ownerKey) {
...
exports.makeZoneFileFromGaiaUrl = makeZoneFileFromGaiaUrl;
/*
