...
async function bundleAsync(projectRoot, options) {
  ...
  if (typeof env.offline === 'undefined') {
    try {
      const expoConfig = ConfigUtils().getConfig(projectRoot, {
    ...
    } catch (_unused) {// Ignore the error thrown by projects without an Expo config.
    }
  }
  ...
  if (!env.offline) {
    ProjectUtils().logInfo(projectRoot, WEBPACK_LOG_TAG, withTag(_chalk().default.green('Offline (PWA) support is not enabled in this build. Learn more https://expo.fyi/enabling-web-service-workers\n')));
  }
}
