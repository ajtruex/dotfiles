...
describe.skip('UserManager', () => {
  ...
  it('should correctly use lock to prevent getting session twice, simulatenously', async () => {
    ...
    UserManager._getProfileAsync = _getProfileSpy;
    const [first, second] = await Promise.all([UserManager.getCurrentUserAsync(), UserManager.getCurrentUserAsync()]);
    expect(_getProfileSpy).toHaveBeenCalledTimes(1); // This shouldn't have changed, but just double check it

    expect(first === null || first === void 0 ? void 0 : first.sessionSecret).toEqual(second === null || second === void 0 ? void 0 : second.sessionSecret);
  });
});
